...
const Login = () => {
  ...
  const handleAuthSuccess = (successMessage: string) => {
    // ◄ REMOVED: localStorage tracking. The browser handles the cookie automatically now!
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // ◄ CLEANED UP URL: Relying on the global axios baseline configuration
      const res = await axios.post("/auth/login", {
        email,
        password,
      });

      // ◄ CHANGED: Check for res.data.success instead of looking for a token string
      if (res.data?.success || res.status === 200) {
        handleAuthSuccess("Credentials verified! Welcome back.");
      } else {
        updateStatus(
          "error",
          "Authentication succeeded, but the server encountered an internal handling issue.",
        );
      }
    } catch (err: any) {
      updateStatus(
